...
function fixReferences ($, fname, sources, files, page) {
  ...
  function fix ($this, attr) {
    ...
    // if absolute markdown file, make link relative to MD file
    if (page.absolute) {
      let parsedUrl = urlParse(page.source)

      // resolve pathName with original Url
      let pathName = dirname(parsedUrl.pathname)
      pathName = resolve(pathName, origUrl)
      ...
      // search sources
        ...
        let searchUrl = urlParse(target)
        let searchSource = searchUrl.host
          .replace('.', '+')
          + searchUrl.pathname
            .replace(/\.md$/, '.html')
            .replace(/README\.html$/, 'index.html')

        let foundSource = find(sources, (obj) => (obj === searchSource)) !== undefined

        if (foundSource) {
          target = relative('/' + dirname(sources[base]), '/' + searchSource) + anchor

        // else just link to external url
        } else {
    ...
    // if relative MD file, try to find existing referencee
    } else {
