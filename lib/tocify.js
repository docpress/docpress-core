...
class Tocify {
  ...
  itemify (text, i) {
    ...
    item.url = declash(item.url, this.urls)

    // set `slug`
    item.slug = slugify(item.url) || 'index'
    item.slug = declash(item.slug, this.slugs, { extension: false })

    // set `headings`
    if (this.files && this.files[item.source]) {
