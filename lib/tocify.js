...
class Tocify {
  ...
  itemify (text, i) {
    ...
    // `source`: Takes care of absolute and relative (../) paths
...
function absolutify (source, root) {
  ...
  // if this is an absolute url
  if (parsedUrl.protocol) {
    // lets check if its a .md file
    let fileExt = parsedUrl.pathname.split('.').pop()

    // return that its an absolute url
    if (fileExt.toUpperCase() === 'MD') {
      return {
        source,
        absolute: true
      }

    // return without source, only url
    } else {
      return {
        source: false,
        url: source,
