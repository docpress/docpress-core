...
function buildIndex (files, ms, done) {
  ...
  // Fetch absolute files
  fetchAbsoluteUrls(indexes.index, files)
    .then(function (res) {
    ...
    .catch(function (res) {
      console.log('Failed getAbsoluteUrls')
      console.log(res)
      //throw new Error('Failed getAbsoluteUrls')
    })
}
...
/**
 * Private: fetch absolute files for index and add to files object
 */

function fetchAbsoluteUrls (index, files) {
  let promises = [];

  each(index, (file, url) => {
    if (!files[file.source] && file.absolute) {
      let source = file.source;

      let promise = fetchAbsoluteUrl(source)
        .then(function (body) {
          files[file.source] = {
            contents: body
          }
...
function fetchAbsoluteUrl (url) {
    ...
    return new Promise((resolve, reject) => {
      fetch(url, { headers: headers })
        ...
        .then(function (jsonData) {
          if (jsonData.content) {
            return base64.decode(jsonData.content)
          } else {
            reject(jsonData)
          }
        })
  ...
  if (parsedUrl.hostname.toLowerCase() == 'bitbucket.org' || parsedUrl.hostname.toLowerCase() == 'www.bitbucket.org') {
    // get authentication details
...
/**
 * Private: convert http://www.github.com/docpress/docpress-core/blob/master/README.md
 *  to https://api.github.com/repos/docpress/docpress-core/contents/README.md?ref=master
 */
function convertGithubUrl (url) {
  let parsedUrl = urlParse(url);

  parsedUrl.set('protocol', 'https:')
  parsedUrl.set('hostname', 'api.github.com')
  ...
  // if this is a regular Github url, change blob and parse ref
  if (pathName[3].toLowerCase() == 'blob') {
    // change blob to contents
    pathName[3] = 'contents'

    // save ref
    ref = pathName.splice(4, 1)

    // add ref to query string
    let parsedQueryString = queryString.parse(parsedUrl.query)
    parsedQueryString.ref = ref
    let stringQueryString = queryString.stringify(parsedQueryString)
    if (stringQueryString) {
      parsedUrl.set('query', '?' + stringQueryString)
    }
  }

  // add /repos to path
  pathName.splice(1, 0, 'repos')

  pathName = pathName.join('/')
  parsedUrl.set('pathname', pathName)

  return parsedUrl.href
}

/**
 * Private: convert https://bitbucket.org/MartinFelis/love-android-sdl2/src/a1aca69aa453ad41f45234c8d7739136637acc72/README.md
 *  to https://bitbucket.org/api/1.0/repositories/MartinFelis/love-android-sdl2/src/a1aca69aa453ad41f45234c8d7739136637acc72/README.md;
 */
function convertBitbucketUrl (url) {
  let parsedUrl = urlParse(url);

  parsedUrl.set('protocol', 'https:')
  parsedUrl.set('hostname', 'bitbucket.org')
  ...
  // add /repos to path
  pathName.splice(1, 0, 'api', '1.0', 'repositories')
