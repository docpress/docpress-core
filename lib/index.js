...
function buildIndex (files, ms, done) {
  const m = memoize.bind(this, this.cache)
  ...
  if (readme) {
    readmeData = files[readme].contents.toString()
  } else {
  ...
  const cacheKey = [docs, readmeData, Object.keys(files)]

  // Build `toc`
  const toc = m(['tocify', cacheKey], () => {
    return tocify(readmeData, files, { docs })
  })

  // Build `index` and `sources`
  const indexes = m(['indexes', cacheKey], () => {
    return indexify(toc, { docs })
  })
...
function renderMarkdown (files, ms, done) {
  ...
  var m = memoize.bind(this, this.cache)
  ...
  each(pages, (page, fname) => {
    ...
    var cacheKey = [contents, mdOptions, fname, sources]
    const html = m(['markdown', cacheKey], () => {
      return md(mdOptions, ms._directory).render(contents)
    })
...
function memoize (cache, keys, fn) {
